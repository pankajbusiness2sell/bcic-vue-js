                       ...
                       <div class="card-header">
                               <div class="d-flex justify-content-between">
                                   <div class="pt-2">
                                       ...
                                       <h5 class="card-title text-nowrap">Review Task Report</h5>
                                   </div>
        ...
        <div class="modal-body" >
             ...
             <!-- <pre> {{  propsData.allRecords  }} </pre> -->
                ...
                <div>
                    ...
                    <button type="button" @click="searchType(index)" v-for="(trackname, index) in GetSystemDD[207]" :key="index" class="btn btn-primary p-2 ms-2">
                        {{ trackname }}
                    </button>
                </div>
               <hr/>
            <div class="d-flex justify-content-between">
                    ...
                    <div>
                        ...
                        <div class="d-inline-block me-2">
                            ...
                            <select class="form-select" v-model="assignadmin"  @change="(event) => quoteassign(event, 2)">
                            <option value="0">Select</option>
                            <option 
                                ...
                                {{ adminname.name }} ({{ adminname.login_status === 1 ? 'Online' : 'Offline' }})
                            </option>
                            </select>
                        </div>
                ...
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                      ...
                      >
                        ...
                        <td>{{ taskdata.job_id }} </td>
                        <td>{{ taskdata.name }}</td>
                        <td>{{ taskdata.email }}</td>
                        <td>{{ taskdata.phone }}</td>
                        
                        <td>
                          <span v-if="taskdata.step === 0" class="text-danger">--</span>
                          <span v-else>{{ taskdata.status?.[taskdata.step] }}</span>
                        </td>
                        <td>{{ taskdata.task_admin_name }}</td>
                        <td>{{ getadminname(taskdata.login_id) || '' }}</td>
                        <td>
                          <span v-if="!taskdata.imgstr || taskdata.imgstr.length === 0" class="text-warning">No Job Types</span>
                          <span v-else>{{ taskdata.imgstr.map(item => item.job_type).join(', ') }}</span>
                        </td>
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const getadminlist = async () => {
                ...
                //         adminlist.value = originalAdminList.value.filter(admin => admin.auto_role === 1);
                //     }
                // };
            ...
            const getsalesTask = async () => {
                loading.value = true;
                try {
                    const formData = {adminid: 'all', taskType: 4};
            ...
            const quoteassign = async (event, typeid) => {
                ...
                const confirmMessage = `Do you want to move Review tasks to ${showtext}?`;
                ...
                const response = await sendData('post', '/review-track-task-assign', formData);
